...
std::optional<std::string> sha256_file(const std::filesystem::path& p) {
    ...
    while (d.size() % 64 != 56)
        d.push_back(0);
    for (int i = 7; i >= 0; --i)
        d.push_back(static_cast<std::uint8_t>(bits >> (i * 8)));
    std::array<std::uint32_t, 8> h{0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
                                   0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19};
    for (std::size_t block = 0; block < d.size(); block += 64) {
        std::array<std::uint32_t, 64> w{};
        for (std::size_t i = 0; i < 16; ++i) {
            auto o = block + i * 4;
            w[i] = (static_cast<std::uint32_t>(d[o]) << 24U) |
                   (static_cast<std::uint32_t>(d[o + 1]) << 16U) |
                   (static_cast<std::uint32_t>(d[o + 2]) << 8U) | d[o + 3];
        }
        for (std::size_t i = 16; i < 64; ++i) {
            auto s0 = std::rotr(w[i - 15], 7) ^ std::rotr(w[i - 15], 18) ^ (w[i - 15] >> 3U);
            auto s1 = std::rotr(w[i - 2], 17) ^ std::rotr(w[i - 2], 19) ^ (w[i - 2] >> 10U);
            w[i] = w[i - 16] + s0 + w[i - 7] + s1;
        }
        auto a = h[0], b = h[1], c = h[2], dd = h[3], e = h[4], ff = h[5], g = h[6], hh = h[7];
